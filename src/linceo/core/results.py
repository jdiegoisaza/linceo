...
class RunResult:
    ...
    findings (ADR §5). `findings` holds the run's findings after
    intra-run, fingerprint-based deduplication (ADR §5, "Deduplicación");
    each tool's own unmerged report survives in `executions`, which is how
    provenance for a deduplicated finding — which tool(s) reported it — stays
    recoverable without `Finding` itself needing to carry more than one
    tool name.
    """
