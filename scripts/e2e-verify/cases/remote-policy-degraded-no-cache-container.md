# docker run --network none ... scan secrets --config <remote>, no cache

**Status:** ok
**Exit code:** 0

```bash
docker run --rm --network none -v '<WORKSPACE>:/workspace' -v '<RUNDIR>/policy-cache-remote-policy-degraded-no-cache-container:/policy-cache' -e SYSTEM_COLLECTIONURI=https://dev.azure.com/juandiego-13/ -e LINCEO_POLICY_CACHE_DIR=/policy-cache ghcr.io/jdiegoisaza/linceo:latest scan secrets --config /workspace/.devsecops/policy-remote.toml
```
